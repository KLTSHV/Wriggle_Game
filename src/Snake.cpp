...
//       stopped(false), totalSegments(numSegments), spawnTimer(0.f)
// {
//     desiredSpacing = segmentSize * 2.0f;

//     // Compute velocity from speed and angle (in radians).
//     velocity.x = speed * std::cos(angle);
//     velocity.y = speed * std::sin(angle);

//     // Time interval between new segments appearing.
...
//     // Release snake after the stop timer
//     stoppedTimer -= deltaTime;
//     if (stoppedTimer <= 0) {
//         stopped = false;
//     }

//     // Make existing segments follow the head
...
//             float smoothing = 0.2f;  // tweak to your taste
//             sf::Vector2f newPos = currPos + (targetPos - currPos) * smoothing;
//             segments[i].setPosition(newPos);
//         }
//     }

//     // Spawn new segments until we have totalSegments
...
//     // Check if all segments are off-screen -> begin fading
...
//     // Build a bounding rectangle that covers all segments:
...
// void Snake::makeStop() {
//     stopped = true;
//     stoppedTimer = DEFAULT_STOP_TIMER;
// }
